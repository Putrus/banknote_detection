...
if __name__ == '__main__':
    while (True):
        ...
        print("8 - Test one banknote on all test images")
        print("9 - Test all banknotes on all test images")
        print("Select option:")
        n = input()
        if n == "1":
            ...
            test.testImage(filename, "69", stop_after_diagnosis, save)
        elif n == "3":
            print("Choose banknote:")
            banknote = input()
            assert (banknote in ("10", "20", "50", "100")), "Wrong banknote"
            ...
            banknote = input()
            assert (banknote in ("10", "20", "50", "100")), "Wrong banknote"
            tests_images = os.listdir("tests")[1:]
            for i in tests_images:
                print("Test image " + str(i) + " for banknote " + str(banknote) + "pln")
                test.testImage(i.replace(".jpg",""), banknote, stop_after_diagnosis, save)
        elif n == "9":
            tests_images = os.listdir("tests")[1:]
            for i in tests_images:
                print("Test image " + str(i))
                test.testImage(i.replace(".jpg",""), "69", stop_after_diagnosis, save)
        else:
            print("Wrong option! Program closed!")
            break;